...
The information collected by Finora will also be presented through financial summaries and spending breakdowns, helping users quickly understand their overall situation. Students will be able to review their income relative to their expenses, examine spending across categories, monitor their budgets, and track progress toward savings goals. This information can help identify patterns that may not be obvious when looking at individual purchases. For example, several relatively small purchases over the course of a month may appear insignificant individually but could add up to a substantial amount when viewed together. Providing a broader view of these patterns can help students recognize specific areas where adjustments could improve their financial situation.

Personal finance and budgeting solutions already exist and offer a range of useful features, including expense tracking, budgeting, transaction categorization, and financial summaries. However, many of these solutions are designed for a broad range of users rather than specifically around the circumstances commonly experienced by college students. Finora is not intended to compete with every capability provided by larger financial platforms. Instead, its purpose is to provide a more focused experience that addresses the financial situations students are likely to encounter. This includes managing income from part-time employment, accounting for financial aid and scholarship funds, recognizing educational expenses, managing limited disposable income, and considering how available funds may need to last throughout an academic semester.
...
Our top priority should be getting the Database set up along with organizing the data the website will be storing.  Income/expense tracking, financial data, data analysis, and budgeting will all need the framework of the database to be in place.  The next item we will need to accomplish will be basic functionality in inputting user data: adding/editing/removing income and expenses into the database, assigning categories to expenses and sources to income, etc.  Financial reports, budget planners, transaction history, the recurring expense tracker, and saving goals will need data to be read before they are able to do anything.  After this point a few avenues will open.  Detailing the financial report, financial calculations, and determining what will be displayed will be needed before the website and GUI can be finalized.  Transaction history will need to be in place before a search and filter feature can be implemented.  And lastly, the budget system will need to be implemented before we can detail category budgets and monthly budgets.
...
An item we will need to implement once the database framework is in place is a method of inputing user data.  Any infographics and income/expense tracking reports we want to display are contingent on data being given.
